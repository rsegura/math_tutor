...
from math_tutor.domain.evidence import EvidenceRecord, Observation, ObservationOutcome
...
def test_age_changes_presentation_but_not_competency() -> None:
    ...
    estimate = SkillEstimate("units-tens", CompetencyState.EXPLORING)
...
def test_one_success_cannot_advance_competency() -> None:
    policy = ProgressionPolicy(min_successes=2, min_distinct_activities=2)
    estimate = SkillEstimate("units-tens", CompetencyState.EXPLORING)
...
def test_policy_cannot_be_configured_to_advance_from_one_success() -> None:
    with pytest.raises(InvalidLearningState, match="at least 2"):
        ProgressionPolicy(min_successes=1)
...
def test_low_confidence_stt_cannot_lower_competency() -> None:
    ...
    estimate = SkillEstimate("units-tens", CompetencyState.INDEPENDENT)
    evidence = (
...
def test_repeated_reliable_failures_propose_review_not_silent_regression() -> None:
    policy = ProgressionPolicy(min_failures_for_review=2)
    estimate = SkillEstimate("units-tens", CompetencyState.INDEPENDENT)
    evidence = (
...
def successful_evidence(evidence_id: str, activity_id: str) -> EvidenceRecord:
    ...
    return EvidenceRecord.initial(evidence_id=evidence_id, observation=observation)
...
def failed_evidence(
    ...
    observation = Observation.from_answer(
        ...
        assistance_level=0,
        min_reliable_stt_confidence=0.75,
    )
    return EvidenceRecord.initial(evidence_id=evidence_id, observation=observation)
